...
class SchoolHolidayRepositoryImpl implements SchoolHolidayRepository {
  ...
  Future<Either<Failure, List<domain.SchoolHoliday>>> getSchoolHolidays({
    ...
    try {
      ...
      return Left(
        Failure(
          message: 'Failed to fetch school holidays: ${e.toString()}',
  ...
  @override
  Future<Either<Failure, List<domain.SchoolHoliday>>> getSchoolHolidaysForRange({
    required String stateCode,
    ...
    try {
      ...
      return Left(
        Failure(
          message: 'Failed to fetch holidays for range: ${e.toString()}',
          stackTrace: StackTrace.current,
        ),
      );
    }
  ...
  Future<Either<Failure, List<domain.SchoolHoliday>>> getCachedHolidays({
    ...
    try {
      ...
      return Left(
        Failure(
          message: 'Failed to get cached holidays: ${e.toString()}',
          stackTrace: StackTrace.current,
        ),
      );
    }
  ...
  Future<Either<Failure, List<domain.SchoolHoliday>>> refreshHolidays({
    ...
    try {
      ...
      return Left(
        Failure(
          message: 'Failed to refresh holidays: ${e.toString()}',
          stackTrace: StackTrace.current,
        ),
      );
    }
